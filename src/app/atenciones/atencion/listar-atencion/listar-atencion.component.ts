...
export class ListarAtencionComponent implements OnInit {
  ...
  constructor(public _doctorService: AtencionService, public _pacienteService: AtencionService) { }
  ...
  closeAdd(send) {
    ...
    console.log(this.nuevo_horario['horaAperturaCadena']);
    var h = this.nuevo_horario['horaAperturaCadena'].split(':');
    this.nuevo_horario['horaAperturaCadena'] = h[0]+h[1];
    var h = this.nuevo_horario['horaCierreCadena'].split(':');
    this.nuevo_horario['horaCierreCadena'] = h[0]+h[1];
    console.log(this.nuevo_horario);
    if (send) {
      this._doctorService.post1(this.nuevo_horario).subscribe(() => {
        console.log(this.nuevo_horario);
        this.getData();
      });
    }
  ...
  closeEdit(send) {
    ...
    if (send) {
      ...
      this._doctorService.put1(this.edit_horario).subscribe(() => {
        this.getData();
      });
    }
    this.edit_horario = {
  ...
  closeDelete(send) {
    if (send) {
      this._doctorService.delete1(this.delete_horario['idPersonaHorarioAgenda']).subscribe(() => {
        this.getData();
      });
    }
    this.delete_horario = {
